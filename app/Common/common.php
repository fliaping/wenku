	...
	//echo  $filename;die;
	 ...
	 if(!file_exists('./' . C('mtceo_attach_path')."docswf/data/".$filename.".php")){ 
	 		if(file_exists($htmlname)){//如果该文档存在,则取出字符串
    	        ...
    	        //$ipstr=str_replace("</body>", '', $ipstr);
				$ipstr=str_replace("<EXAMPLE>",$filename, $ipstr);
	 		...
	 		//如果没有该文件则显示未生成预览
	 		$ipstr="<div class=\"stl_01 mod reader-page complex reader-page-\"><div style=\"height:300px;font-size:30px;color:red;padding-top:150px;font-weight:bolder;text-align:center;\">Sorry,该文档尚未生成预览</div></div>";
	 	}	
	 }//如果不存在缓存文件的判断结束
	 else{
