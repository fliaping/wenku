...
class baseAction extends Action
{
    ...
    protected function _upload($file, $dir = '', $thumb = array(), $save_rule='uniqid') {
        ...
        if ($result = $upload->uploadOne($file)) {
            $source = $upload_path.$file['name'].$file['extension'];
            $destination = $upload_path.'/files/'.$file['name'].$file['extension'];
            if($rr= copy($source,$destination)) return array('error'=>1, 'info'=>$upload->getErrorMsg());
